...
def _build(structure: dict, out_path: Path, *, template: str = "simple") -> dict:
...
def generate_from_html(html_path: Path, out_path: Path, *, template: str = "simple") -> dict:
    ...
    result = _build(structure, out_path, template=template)
    result["mode"] = "html"
    return result
...
def generate_from_url(url: str, out_path: Path, *, template: str = "simple", cache_dir: Path | None = None) -> dict:
...
def generate_from_image(image_path: Path, out_path: Path, *, title: str = "My Blog", template: str = "from-image") -> dict:
